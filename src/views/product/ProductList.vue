                ...
                <template #overlay>
                  <a-menu>
                    ...
                    <a-menu-item>
                      ...
                      <a-modal
                        v-model:visible="open2[record.key]"
                        @cancel="handleCancel2(record.key)"
                        title="保险详情"
                        width="100%"
                        wrapClassName="full-modal"
                        cancelText="关闭"
...
<script setup>
  ...
  const isObjectEmpty = (obj) => {
    ...
    for (const key of Object.keys(obj)) {
      if (key !== 'id' && key !== 'key' && key !== 'recNum' && key !== 'soldNumber' && key !== 'proPageImg' && key !== 'posters' && !obj[key]) {
        return false;
      }
    }
    return true;
  ...
  const customProPageImgRequest = async (e) => {
    ...
    uploadImg(data).then((res) => {
      ...
      e.file.url = res[0];
      formState.value.proPageImg = res[0];
      e.onSuccess(res[0], e);
    });
  };
  const customPostersRequest = async (e) => {
    ...
    uploadImg(data).then((res) => {
      e.onProgress({ percent: 100 });
      e.file.url = res[0];
      formState.value.posterImg = res[0];
      e.onSuccess(res[0], e);
    });
  };
